...
4. Filtros opcionales por materia, aula y **profesor** (por id; decisión T-42, vista personal de la agenda de ese profesor ese día), más `q` (decisión T-36): búsqueda por nombre de alumno **o** de profesor, con `terminosDeBusqueda` como en el resto de la API. Todas las palabras tienen que coincidir en la `busqueda` del mismo (alumno o profesor), nunca mezcladas entre los dos. **Si además se manda `profesorId`, `q` busca solo por alumno** (T-42: buscar también por profesor no aportaría nada, ya está fijo). En `turnos.repository.listarAgenda`, la condición de fecha (`condicionTurnoEnFecha`) y la de búsqueda van como ramas separadas de un `AND` explícito, porque las dos usan la clave `OR` y una pisaría a la otra si se mezclaran en el mismo objeto; `profesorId` va dentro de la rama de `bloqueAgenda`, junto con `diaSemana` y `aulaId`.
