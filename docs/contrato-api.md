...
| `GET /api/v1/turnos/profesores?fecha?`                                        | Profesores con al menos un turno `ACTIVO` en `fecha` (`YYYY-MM-DD`; sin ella, hoy): `[{ "id", "apellido", "nombre" }]`, ordenados por apellido y nombre. Para el filtro de profesor de la agenda. Tampoco filtra por si el profesor (su `Usuario`) sigue activo hoy                                                                                                                                  |
