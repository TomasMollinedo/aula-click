...
| `zod.ts`        | `dni`: se limpian puntos y espacios y luego se valida 7 u 8 dígitos; **se guarda solo con dígitos**. `email`: trim, minúsculas, email válido de hasta 254 caracteres (medidos después del trim). `telefono`: trim; solo dígitos, espacios, `+`, `-` y paréntesis; entre 8 y 20 caracteres con al menos 8 dígitos; **se guarda como lo escribió el usuario**. `textoRequerido(max)`: trim, entre 1 y `max` (lanza `RangeError` si `max` no es un entero >= 1). `nombrePersona(max)`: nombre o apellido de una persona; `textoRequerido(max)` más solo letras (con tildes, ñ, ü), espacios, apóstrofos y guiones, con al menos una letra (rechaza números y símbolos). `fechaISO`: `YYYY-MM-DD` y fecha real de calendario; sale como string. `horaHHmm`: `HH:mm` de 00:00 a 23:59. `horaAMinutos(hora)` / `minutosAHora(minutos)` (0 a 1439; lanzan `RangeError` si la entrada es inválida: `minutosAHora(1440)` también lanza). Los mensajes de error están en español. |
