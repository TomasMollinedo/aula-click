...
- `requireRole(...roles)` recibe `string[]` mientras `shared/actor.ts` no exista; cuando exista, pasa a recibir `Role[]`, así un rol mal escrito no compila.
...
- La regla de ESLint que impide a `src/server/shared/**` importar features o Prisma ya está en `eslint.config.mjs`: al construir `shared/`, confirmar en positivo que una feature puede importar de `shared`.
