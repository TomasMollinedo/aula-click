...
3. **Solo los repositories usan Prisma.** Excepciones: `src/lib/prisma.ts`, que lo instancia, `src/lib/auth.ts`, que lo pasa al adaptador de Better Auth, `prisma/seed.ts`, el seed de desarrollo, y los `*.condiciones.ts` de una feature, que solo importan tipos de Prisma y consultan con el cliente que les pasa un repository (decisión T-39).
...
6. **Piezas "A construir".** Lo marcado **A construir** en los docs (por ejemplo `AppShell` o la auditoría completada por el repository) todavía no existe. Si una tarea lo necesita, avisar a la persona antes de crearlo: se construye una sola vez, en un PR propio, con sus tests y según su especificación. Nunca se inventa una versión propia dentro de una feature.
...
| Prisma                     | importar `@/lib/prisma` o `@/generated/*`                                                                                                                                           | `src/server/**` y `src/app/api/**`, salvo `*.repository.ts` y `*.condiciones.ts` (en `src/lib/` sí se permite)                                                                                                 |
| Entre features de API      | importar de otra feature cualquier archivo que no sea su `*.repository` o sus `*.condiciones` (con alias `@/server/features/*/…` o `../…`)                                          | `src/server/features/**`, salvo `__tests__/` (los tests pueden importar tipos de la validation de otra feature, pero no su service, controller ni routes); lo prueba `shared/__tests__/eslint-limites.test.ts` |
| Repository → repository    | que un `*.repository.ts` o `*.condiciones.ts` importe el repository de otra feature (lo que necesite de otra feature dentro de su transacción lo toma de sus `*.condiciones`)       | `src/server/features/**/*.repository.ts` y `*.condiciones.ts`                                                                                                                                                  |
| `shared` → features        | importar la carpeta `@/server/features`, `@/lib` (Prisma, auth, storage), `@/config` o `@/generated`, con alias o con ruta relativa (`../features/…`, `../../lib/…`), y el frontend | `src/server/shared/**` (lo prueba `shared/__tests__/eslint-limites.test.ts`)                                                                                                                                   |
