...
| Autenticación, roles y sesión en la API          | `src/lib/auth.ts`, `src/server/middlewares/auth.ts`, `src/proxy.ts`                                                                                                       | Better Auth, `requireAuth()`/`requireRole()` y la redirección a `/login`      |
...
| Página, layout, Sidebar o Header                 | `docs/arquitectura-frontend.md` (Roles y route groups; Layout), `src/app/layout.tsx`, `src/app/(personal-mesa-entradas)/layout.tsx`, `src/components/layout/`             | Route groups sin rutas repetidas; Header y Sidebar vía `AppShell`             |
