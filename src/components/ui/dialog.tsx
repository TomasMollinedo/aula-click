...
function DialogOverlay({ className, ...props }: ComponentProps<typeof DialogPrimitive.Overlay>) {
  return (
    <DialogPrimitive.Overlay
      ...
      className={cn(
        'bg-tinta/50 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fixed inset-0 z-50',
        className,
      )}
      {...props}
    />
  ...
  return (
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          // Alto máximo: un contenido más alto que la pantalla no estira el diálogo dejando los
          // botones del pie inalcanzables; si no entra, scrollea el diálogo. Es una red de
          // seguridad: un diálogo de confirmación no debería llegar a ese largo.
          'border-border bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-1/2 left-1/2 z-50 flex max-h-[calc(100dvh-2rem)] w-full max-w-lg -translate-x-1/2 -translate-y-1/2 flex-col gap-4 overflow-y-auto rounded-lg border p-6 shadow-lg duration-200',
          className,
        )}
        onInteractOutside={(e) => {
